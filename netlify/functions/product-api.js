...
exports.handler = async (event, context) => {
  ...
  console.log('Processing request:', { path, method, body, query });
...
async function getGameStatus(gameId) {
  try {
    ...
    console.log('Game status retrieved:', { stage: game.stage, players: game.players?.length });
...
async function progressGame(body) {
  try {
    ...
    const timeSinceStart = Math.floor((currentTime - gameStartTime) / 1000); // seconds
    ...
    console.log('Game progress check:', { 
      stage: game.stage, 
      timeSinceStart, 
      currentTimer: game.lobby_timer || game.round_timer 
    });
    ...
    if (game.stage === 'lobby') {
      ...
      if (newLobbyTimer <= 0) {
        ...
        // Make bots choose conjoint options randomly
      ...
      if (newRoundTimer <= 0) {
        ...
        const players = game.players.map(player => {
          let score = Math.floor(Math.random() * 100); // Simple random score for now
          return { ...player, score };
        });
